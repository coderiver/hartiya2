head.ready(function() {
	...
	$(document).scroll(function(event) {
		...
		// check if layout is
		if ( wrapper.parents('.l_article').length !== 0 && $(window).width() < 1150
			|| $('.l-main').height() < sidebar.height() ) {
			return false;
		};

		if (wrapper.children('.l-top').length !== 0 && $(window).width() < 1150) {
			sidebarBottomPosition = wrapper.offset().top + $('.l-top').height() + 50 + sidebar.height(),
			wrapperTopPosition    = wrapper.offset().top + $('.l-top').height() + 50;
		};
